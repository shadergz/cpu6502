...
std::pair<uint64_t, uint64_t> cpu6502::clock (size_t cycles_count, uint64_t &executed_cycles)
{
	...
	for (; cycles_count > 0; executed++) {
		try {
			...
			std::cerr << std::format ("Stopped by a invalid instruction opcode {:#x} during the decode event",
...
{
	std::cout << std::format ("6502 microprocessor informations:\nPC = {:#x} : STACK POINTER: {:#x}", 
		m_pc, (uint16_t) m_s | 0x100) << std::endl;
	std::cout << std::format ("CPU register:\nA = {:#x} : X = {:#x} : Y = {:#x}", m_a, m_x, m_y) << std::endl;
	std::cout << std::format ("CPU RS:\nCARRY = {}", 
		getflag (flags::CARRY)) << std::endl;
